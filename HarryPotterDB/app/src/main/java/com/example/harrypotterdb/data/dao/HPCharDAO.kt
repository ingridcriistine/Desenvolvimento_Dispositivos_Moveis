...
class HPCharDAO (private val context: Context) {
    ...
    fun addChar(hpChar: HPChar) : Long {
    ...
    fun getCharById(id: Int): HPChar? {
        val db = dbHelper.readableDatabase
        val cursor: Cursor = db.query(
            DBHelper.TABLE_NAME, null, "id = ?", arrayOf(id.toString()), null, null, null)
    ...
    fun updateChar(hpChar: HPChar): Int {
        ...
        val rowsAffected = db.update(
            DBHelper.TABLE_NAME, values, "id = ?", arrayOf(hpChar.id.toString())
        )
        db.close()
        return rowsAffected
    }
    ...
    fun deleteChar(id: Int): Int {
        val db = dbHelper.writableDatabase
        val rowsDeletec = db.delete(
            DBHelper.TABLE_NAME, "id = ?", arrayOf(id.toString())
        )
        db.close()
        return rowsDeletec
    }
